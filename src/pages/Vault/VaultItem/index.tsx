...
import { StakeModal, WithdrawFromSebVaultModal, WithdrawFromVestingVaultModal } from '../modals';
...
export const VaultItemUi: React.FC<VaultItemUiProps> = ({
  ...
  return (
    <>
      ...
      {activeModal === 'withdraw' && poolIndex === undefined && stakedTokenId === TOKENS_EVMOS.seb.id && (
        <WithdrawFromSebVaultModal handleClose={closeActiveModal} />
      )}
...
const VaultItem: React.FC<VaultItemProps> = ({
  ...
  const onWithdraw = async () => {
    ...
    if (!account?.address) {
      throw new NError({ type: 'interaction', code: 'accountError' });
    }
  ...
  return (
    <VaultItemUi
      ...
      isWithdrawLoading={isClaimRewardLoading} //isWithdrawFromVrtVault
      {...vaultItemUiProps}
    />
  );
};
