...
import { Asset, TokenId, NTokenId } from 'types';
import {
  convertTokensToWei,
...
export const SupplyWithdrawUi: React.FC<SupplyWithdrawUiProps & SupplyWithdrawProps> = ({
  ...
  const nErc20TokenContractAddress = getNErc20Token(asset.id as NTokenId).address; 
  ...
  const tokenInfo: LabeledInlineContentProps[] = asset
    ? [
        ...
        },
        {
          ...
          iconName: 'mia' as IconName,
          children: formatToReadablePercentage(asset.miaSupplyApy),
        },
      ]
    : [];
      ...
      <div className={className} css={styles.container}>
        <ConnectWallet message={message}>
          {asset && (
            <EnableToken
              nTokenId={asset.id}
              spenderAddress={nErc20TokenContractAddress}
              title={title}
              tokenInfo={tokenInfo}
            >
              <SupplyWithdrawForm
...
const SupplyWithdrawModal: React.FC<SupplyWithdrawUiProps> = props => {
  const { asset, assets, isMiaEnabled, onClose, ...rest } = props;
  ...
  const { data: nTokenBalanceData } = useGetNTokenBalanceOf( 
    { accountAddress, nTokenId: asset.id as NTokenId },
  ...
  const onSubmitWithdraw: AmountFormProps['onSubmit'] = async value => {
    ...
    if (amountEqualsSupplyBalance && nTokenBalanceData?.balanceWei) {
      const res = await redeem({ amountWei: new BigNumber(nTokenBalanceData.balanceWei) });
