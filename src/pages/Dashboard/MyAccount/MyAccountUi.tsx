...
export interface MyAccountUiProps {
  ...
  onMiaToggle: (newValue: boolean) => void;
  className?: string;
}
...
export const MyAccountUi = ({
  ...
  const handleMiaToggleChange: ToggleProps['onChange'] = (_event, checked) => onMiaToggle(checked);
  ...
  return (
    <Paper css={styles.container} className={className}>
      <div css={[styles.row, styles.header]}>
        ...
        <div css={styles.apyWithMia}>
          <Tooltip css={styles.tooltip} title={t('myAccount.apyWithMiaTooltip')}>
          ...
          <Typography
            ...
            variant="small1"
            component="span"
            css={styles.apyWithMiaLabel}
          >
            {t('myAccount.apyWithMia')}
          </Typography>

          <Toggle css={styles.toggle} value={isMiaEnabled} onChange={handleMiaToggleChange} />
        </div>
      </div>
