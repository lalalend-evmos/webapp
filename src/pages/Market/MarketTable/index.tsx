    ...
    () =>
      markets.map(market => [
        ...
        },
        {
          ...
          render: () => (
            <LayeredValues
              topValue={formatToReadablePercentage(market.supplyApy.plus(market.supplyMiaApy))}
              bottomValue={formatToReadablePercentage(market.supplyMiaApy)}
            />
          ),
          value: market.supplyApy.plus(market.supplyMiaApy).toFixed(),
          align: 'right',
        },
        {
          key: 'totalBorrows',
        ...
        },
        {
          ...
          render: () => (
            <LayeredValues
              topValue={formatToReadablePercentage(market.borrowApy.plus(market.borrowMiaApy))}
              bottomValue={formatToReadablePercentage(market.borrowMiaApy)}
            />
          ),
          value: market.borrowApy.plus(market.borrowMiaApy).toFixed(),
          align: 'right',
        },
        {
          key: 'liquidity',
...
const MarketTable = () => {
  const { data: { markets } = { markets: [], dailyMiaWei: undefined } } = useGetMarkets({
    placeholderData: { markets: [], dailyMiaWei: undefined },
  });
