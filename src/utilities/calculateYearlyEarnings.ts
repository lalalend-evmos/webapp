...
export const calculateYearlyEarningsForAssets = ({
  ...
  dailyMiaDistributionInterestsCents: BigNumber;
}) => {
  ...
  if (yearlyEarningsCents && isMiaEnabled) {
    const yearlyMiaDistributionInterestsCents =
    dailyMiaDistributionInterestsCents.multipliedBy(DAYS_PER_YEAR);

    yearlyEarningsCents = yearlyEarningsCents.plus(yearlyMiaDistributionInterestsCents);
  }
