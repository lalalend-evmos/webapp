...
const Title: React.FC = () => {
  ...
  const marketDetailsMatch = useRouteMatch<{ nTokenId: NTokenId }>(Path.MARKET_DETAILS);
  ...
  if (marketDetailsMatch) {
    const { nTokenId } = marketDetailsMatch.params;
    const token = getToken(nTokenId);

    return (
      <BackButton>
        <Icon name={nTokenId} css={styles.backButtonTokenIcon} />
